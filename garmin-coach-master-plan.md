...
**Version:** 3.1 — Claude API replaces Antigravity/agy  
**Stack:** n8n (Docker) · python-garminconnect · Supabase (training schema) · Claude API (claude-opus-4-8) · Telegram Bot · iOS Shortcuts  
**Repository:** `/Users/alitkovsky/Documents/GitHub/training`
...
The workflow:
1. Claude API generates an adjusted weekly plan → writes to training/plan.md
...
garmin-api /analyze (08:15) — AI ANALYSIS
  ├─ daily_analysis.py reads today.json + 28-day Supabase history
  ├─ claude-opus-4-8 with adaptive thinking via Anthropic API
  └─ Writes reports/YYYY-MM-DD.md + Supabase training.daily_reports
...
n8n → garmin-api /analyze weekly (Monday 08:30) — WEEKLY REVIEW
  └─ Full week analysis via Claude API → annotates training/plan.md
...
## Phase 2 — Training Plan + Claude Code Scheduled Analysis
**Goal:** Daily coaching report generated by a Claude Code cloud routine from real Supabase data.
**Duration:** 1–2 days
...
- [ ] Review `prompts/daily-analysis.md` — adjust coaching context for your race goals
  (This file is read at runtime by the Claude Code cloud agent from the GitHub repo)

### 2.3 — Claude Code Scheduled Routine

A cloud agent is scheduled to run at 08:15 Europe/Berlin daily via claude.ai/code/routines.
It reads `prompts/daily-analysis.md` from the repo, queries Supabase via MCP, and upserts
the report to `training.daily_reports`.

- [ ] Confirm the routine exists: https://claude.ai/code/routines → "Garmin Daily Coaching Analysis"
- [ ] Run it manually once to verify it can reach Supabase and produces a report
- [ ] Check report appears in `training.daily_reports` and `reports/YYYY-MM-DD.md`

No ANTHROPIC_API_KEY or local Python changes needed. The Claude Pro subscription drives the agent.

**Phase 2 done when:** training.daily_reports row exists every morning with readiness_score and report_md.
...
1. Read updated training/plan.md (after Claude API weekly review annotations)
...
### 5.4 — Claude API: garmin-workout-scheduler

A Python script `scripts/garmin_workout_scheduler.py` using claude-opus-4-8:
- Reads the weekly plan from training/plan.md
- Identifies session types and dates
- Outputs structured JSON: {date, session_type, workout_library_id}
- n8n calls garmin-api /schedule endpoint which triggers the calendar push
...
- [ ] Weekly review workflow (Monday 08:30): Claude API analyses full week via garmin-api /analyze
- [ ] Plan annotation: Claude API marks training/plan.md with <!-- AI: --> comments;
  you approve/reject via Telegram /approve or /reject commands
- [ ] Monthly mesocycle review: Claude API evaluates training block, proposes next phase
- [ ] Auto-commit reports to git via n8n Execute Command (git add/commit/push)
- [ ] Backup report delivery: if report file missing by 09:00, n8n sends Telegram alert
...
```
your-repo/
├── AGENTS.md                              # Claude Code project constitution + coaching context
├── training/
...
│   ├── garmin_api.py                      # HTTP sidecar for n8n (fetch + analyze)
│   ├── garmin_bulk_import.py              # Historical bulk import
│   ├── daily_analysis.py                  # Claude API coaching analysis
│   ├── garmin_calendar_push.py            # Phase 5: push to Garmin calendar
│   ├── Dockerfile                         # garmin-api container
│   ├── requirements.txt
│   └── .env                               # (gitignored)
├── prompts/
│   ├── daily-analysis.md                  # coaching context for daily_analysis.py
│   └── weekly-review.md                   # coaching context for weekly review
...
| Phase 2 | training/plan.md + Claude API daily analysis (daily_analysis.py) | Days 3–4 |
...
- LM Studio / local LLM — Claude Code cloud routine on Claude Pro; no local model needed
- Antigravity/agy — replaced by Claude Code cloud routine (Claude Pro subscription)
